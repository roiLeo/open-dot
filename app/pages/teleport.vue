<template>
  <div>
    ...
    <UFormGroup label="From chain">
      <USelectMenu v-model="state.from" :options="fromChains" size="xl" @change="handleFromChange" />
    </UFormGroup>
...
const initializeAssetOptions = (fromChain: ChainId) => {
  ...
  state.asset.selected = state.asset.options[0] as AssetId
}
...
const initializeToOptions = () => {
  ...
  state.to.selected = state.to.options[0] as ChainId
}
